...
function fazerDoacao() {
  const mensagem = `Olá! 💕\n\nAchei lindo o *Projeto Ternura* e gostaria muito de apoiar fazendo uma doação de materiais (lãs, linhas, roupinhas, toalhas ou itens de higiene para o enxoval).\n\nComo posso fazer para entregar a vocês?`;
  const mensagemCodificada = encodeURIComponent(mensagem);
  const linkWhatsapp = `https://api.whatsapp.com/send?phone=${TELEFONE_WHATSAPP}&text=${mensagemCodificada}`;
  window.open(linkWhatsapp, "_blank");
}